...
export const checkOptRestrictions = async (
  email: string,
  next: NextFunction
) => {
...
export const trackOtpRequests = async (email: string, next: NextFunction) => {
  ...
  if (otpRequests >= 2) {
    await redis.set(`otp_spam_lock:${email}`, "locked", "EX", 3600); // 1 hr
    throw new ValidationError(
...
export const handleForgotPassword = async (
  ...
  try {
    ...
    if (!email) throw new ValidationError("Email is requried!");
...
export const verifyForgotPasswordOtp = async (
  ...
  try {
    ...
    if (!email || !otp) throw new ValidationError("Email & OTP are required");

    await verifyOtp(email, otp, next);

    return res.status(200).json({
      message: "OTP verified. You can now reset you password.",
    });
  } catch (error) {
    return next(error);
  }
